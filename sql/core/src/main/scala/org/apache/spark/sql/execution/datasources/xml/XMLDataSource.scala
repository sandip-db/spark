...
import org.apache.spark.sql.execution.datasources.xml.util.{InferSchema, XmlUtils}
...
object MultiLineXMLDataSource extends XMLDataSource {
  ...
  override def infer(
    ...
    val sampled: RDD[PortableDataStream] = XmlUtils.sample(xml, parsedOptions)

    val tokenRDD = sampled.flatMap { portableDataStream =>
